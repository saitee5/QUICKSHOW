...
import { StarIcon, CheckIcon, DeleteIcon, XIcon } from 'lucide-react';
...
const AddShows = () => {
  ...
  const handleAddShow = async () => {
    ...
    if (!showPrice || Object.keys(dateTimeSelection).length === 0) {
      return alert("Please add show price and at least one date/time");
    }
    ...
    // Backend call goes here, e.g.:
    // await axios.post('/api/admin/add-show', showsInput)
      ...
      <div className='mt-6'>
        ...
        <div className='inline-flex gap-2 border border-gray-600 p-1 pl-3 rounded-lg'>
          <input
            ...
            className='outline-none bg-transparent text-sm rounded-md'
          />
          <button
            onClick={handleDateTimeAdd}
            className='bg-primary/80 hover:bg-primary px-3 py-2 text-sm rounded-lg cursor-pointer'
          >
            Add Time
          </button>
        </div>
